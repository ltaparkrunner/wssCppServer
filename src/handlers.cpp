...
// 4. Полная реализация тяжелого handle_path_inf_request
...
//             std::string s3_endpoint = cfg_.get_s3_endpoint(); 
//             std::string bucket_name = cfg_.get_bucket_name();
//             int s3_expires = cfg_.get_s3_expires();
...
//                         // Вызов другого метода этого же класса
...
//             // ... Остальные кейсы (папка без слэша, файл, not exist) из предыдущего ответа ...
//             // Везде внутри лямбды "this->" и "send_not_exist_response_async" будут работать корректно, 
//             // так как это код метода класса WssSession.
